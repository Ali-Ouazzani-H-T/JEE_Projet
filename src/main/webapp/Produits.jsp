...
<%@include file="header.jsp" %>
  <h1>Product List</h1>
  <table id="product-table" class="table table-striped table-bordered table-hover table-dark">
    <thead>
      <tr>
        <th>ID</th>
        <th>Libelle</th>
    ...
    <tbody>
      ...
      %>
        <tr>
          ...
          <td><%= categorie.getLibelléCat() %></td>
          <td ><button class="btn btn-success edit-product" onclick="editProduct(<%= product.getId_produit() %>)" data-product-id="<%= product.getId_produit() %>">Edit</button></td>
          <td ><button class="btn btn-danger delete-product" onclick="deleteProduct(<%= product.getId_produit() %>)" href="Suprimer.do?id=${p.id}" data-product-id="<%= product.getId_produit() %>">Delete</button> </td>
        </tr>
      <% } %>
    </tbody>
  </table>
